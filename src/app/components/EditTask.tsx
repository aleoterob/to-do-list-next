...
const EditTask: React.FC<EditTaskProps> = ({
  ...
  useEffect(() => {
    if (task) {
      ...
      setSelectedDate(task.selectedDate);
    }
  }, [task]);
  ...
  const handleSave = () => {
    ...
    if (task) {
      onSave(task.taskId, taskTitle, taskDescription, selectedDate);
      onClose();
    }
  };
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-transparent z-50">
      <div className="bg-white p-6 rounded-lg shadow-lg w-96 transform transition-opacity duration-300 opacity-100">
        <h2 className="text-xl font-semibold mb-4 text-gray-600">Edit Task</h2>
        ...
        <div className="mb-4">
          ...
          >
            {selectedDate ? `Change Date: ${selectedDate}` : "Pick a Date"}
          </button>
          <input
            type="date"
